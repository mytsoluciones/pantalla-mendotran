...
const LlegadasMultiArrival = () => {
    ...
    //todo Acá debería consultar los datos cada por ejemplo 40 segundos y le doy tiempo para que los muestre cada un minuto
    useEffect(() => {
    ...
    useEffect(() => {
        const intervalLlegada = setInterval(()=> {
            console.log('------------------- use-effect getDatos ----------------------------')
            if(paradas){
            getDatos()
            }else{
                console.log("no yet")
    ...
    const requestData = async () =>{
        ...
        if(urls){
            ...
            try{
                ...
                //? Llenamos el arreglo que contendrá los id de las paradas 
                //setArriId([]);
                for(let i=0;i<data.length;i++){
                    setArriId(arrIdPrev=>[...arrIdPrev, data[i].data.references.stops[0].name]);
                }
                console.log("3) Ids de las paradas consultadas: ");
                console.log(arriId);
                console.log("***********************");
                //? Si tenemos datos, entonces armamos el arreglo final
                //if((arriId.length!==0) && (data.length!==0)){
                if(data!==0){
                    //setParadasPrev([]);
                    //setParadas([]);
                    for(let m=0;m<data.length;m++){
                        //!esto anda pero necesito un paso previo para meter los datos como un solo arreglo
                        setParadasPrev(prevParada => [...prevParada, data[m].data.entry.arrivalsAndDepartures.map(objeto => ({
                          ...objeto,
                          nuevoElemento: arriId[m]
                        }))])
                    }
                    const prueba = paradasPrev.flatMap(subArray => Object.values(subArray));
                    setParadas(prueba)
                }else{
                    console.log("kaine dato mine KING")
                }
                setStart(true);
            }catch(error){
    ...
    const getDatos = async () =>{ 
        ...
        //     setUrls(urlsPrev=>[...urlsPrev, urlName])
        // }
        //!descomentar para volver a cero
        // setArriId([]);
        // setParadasPrev([]);
        // setParadas([]);
